...
//     fallback-coverage.baseline.json — compared over the cases the run and the
...
//     is still caught.
//   - A REFRESH test (skipped in CI) rewrites docs/fallback-coverage.md and the
...
const BASELINE_COMMENT =
  ...
  "fallback-coverage.test.js and explained in docs/fallback-coverage.md. criticalFieldsPct/" +
...
test("fallback coverage has not regressed below the high-watermark", (t) => {
  ...
  if (gate.removed.length) {
    const msg =
      ...
      `re-anchored in your working tree — commit the updated test/integration/fallback-coverage.baseline.json.`;
  ...
  for (const key of GATED) {
    assert.ok(
      ...
      `Fallback ${key} dropped to ${gate.current[key]}% over the ${gate.shared.length} cases shared ` +
        ...
        `the dedicated sources — see docs/fallback-coverage.md and the value differences printed above. ` +
        `If this is an intentional, reviewed change, lower ${key} in ` +
        `test/integration/fallback-coverage.baseline.json.`
    );
  }
});
