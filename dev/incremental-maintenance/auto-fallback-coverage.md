...
## Precondition — run the guard script first

**Before anything else**, run the precondition guard:

```sh
bash dev/incremental-maintenance/auto-fallback-coverage-precondition.sh
```

If it exits **non-zero**, **stop the whole routine** — nothing meaningful changed
in the last 24 hours (it printed the reason); no baseline run, no branch, no PR,
nothing to log. If it exits **0**, proceed to the baseline run below.

The guard's rationale — why an idle repo should skip, what counts as a
"meaningful" change, and why it errs toward running — lives in that script's
header comments. Tune the gate by editing the script, not this doc.
