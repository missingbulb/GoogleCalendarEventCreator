...
const { ICON_COVERAGE } = require("./icon-coverage");
...
function managedLine(id, kind) {
  ...
  if (kind === "icon") {
    // The icon image lives beside this doc (docs/extension-icon-<case>.png), keyed
    // to the leaf by icon-coverage.js, and is generated by the icon snapshot test.
    const state = ICON_COVERAGE[id];
    return `![extension-icon-${state}](extension-icon-${state}.png) ${marker(id)}`;
  }
