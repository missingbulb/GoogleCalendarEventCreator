...
folder: `dev/requirements/infra/data/fetch-page.js` (also used by `refresh-cache`), `extension/config.js` /
...
3. **Probes the event URL** (`probe-url.js`): fetches it the same way the recorder
   will (shared `dev/requirements/infra/data/fetch-page.js` — browser headers + retries) and decides
...
- **add-a-case mode:** the extracted values in the new
  `dev/requirements/extractors/custom/<base>-<issue>.json`, and — if the agent edited the
