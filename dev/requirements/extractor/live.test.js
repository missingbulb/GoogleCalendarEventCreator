...
// (dev/create-extractor/phase1-prepare.sh's scraperapi_fetch, which
...
// dev/create-extractor/phase1-prepare.sh's scraperapi_fetch). Run the suite
...
for (const file of caseFiles) {
  ...
  test(`${testCase.description || file} — ${url}`, () => {
    ...
    assert.ok(
      ...
      `Missing cached HTML for "${name}". It's recorded by the auto-extractor pipeline (an extractor-request issue), or by hand via ScraperAPI — see dev/create-extractor/phase1-prepare.sh`
    );
