...
- **Let each case declare how it's handled, with a default — don't classify it from a parallel source.** When items in a test/plugin system need per-item handling (which renderer produces a snapshot, which verifier checks it, whether it's a placeholder), have the item carry that as its own field — with a sensible default so the common case stays terse — rather than tagging it in a separate spec/doc or a side manifest keyed by id. One source of truth means adding an item can't desync from its classifier, and it collapses several parallel routing systems into one dispatcher. (A UI-snapshot system folded `_(icon)_` / `_(behavior)_` / `_(TBD)_` spec tags **and** a behavior-coverage manifest into a single `kind` (default `"popup"`) + `tbd` flag on each case, deleting the parallel parser/manifests.)
