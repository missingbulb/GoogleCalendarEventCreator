...
| `dev/routines/create-extractor/3-prepare.sh` | Stage 1 of the create-extractor routine: branch, record the event page (inline curl→ScraperAPI, `scraperapi_fetch`), scaffold, prove a green baseline, commit |
...
| `dev/routines/create-extractor/` | The create-extractor routine in one folder: `routine.md` (the label-wired agentic spec) driving four ordinal stage scripts — `1-preconditions.sh` (env gate), `2-triage.js` (route + names via the sources' real `matches()`), `3-prepare.sh` (record + scaffold + baseline), `4-postconditions.sh` (scope + quality + re-verify) — plus the helpers they call (`resolve-source.js`, `extractor-naming.js`, `scaffold.js`, `case-quality.js`, `attach-sample-url.js`, `scraperapi-fetch.sh`). One Claude Code session owns every stage; the agent does the judgment + GitHub I/O |
