...
| Checks | [test-offline-list-sync](test-offline-list-sync.mjs) · [custom-sources-flat](custom-sources-flat.mjs) · [npm-test-glob-coverage](npm-test-glob-coverage.mjs) · [pipeline-site-agnostic](pipeline-site-agnostic.mjs) · [regen-artifacts-merge-ours](regen-artifacts-merge-ours.mjs) · [merge-ours-generated-only](merge-ours-generated-only.mjs) (+ red-first fixtures in [pack.test.mjs](pack.test.mjs), run by this repo's own `npm test`) |
| Daily tasks | none |
...
| Only generated files on that driver | **hardcoded** (`merge-ours-generated-only` — the same list's other direction; keeps the part-authored gallery off it) |
