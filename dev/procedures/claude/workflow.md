...
**both** `extension/manifest.json` and `package.json` (they must stay in sync), update the `value` in `dev/tools/test/uber/shared_constants/version-sync.json` to match, on a
