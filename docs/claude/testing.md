...
- **The snapshot renderer inlines CSS with NO selector specificity — last
  declaration wins.** A property set on both a base class and a variant resolves
  to whichever rule is *later in the file*, not the more specific one. Put an
  overriding property only on the variant, never also on the base — a base
  `.e-year` background overrode `.e-year.past` and painted every past pill red in
  the snapshot (right in a real browser). (#274)
