// Per-leaf snapshot for requirement 4.4: a single-showing card is one whole-card click target.
// The filename (req-4.4) is the link; build-requirements-gallery.js embeds this
// image inline beneath 4.4 in docs/uiRequirements.md.
"use strict";

module.exports = {
  description: "a single-showing card is one whole-card click target",
  data: { supported: true, events: [{ title: "Neighborhood Mixer", start: "2026-06-19T19:00:00", location: "The Annex, Brooklyn" }] },
  listing: "none",
};
