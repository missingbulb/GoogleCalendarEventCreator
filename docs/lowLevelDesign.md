...
| `test/ui/refresh-popup-snapshots.js` | Regenerates the committed popup snapshots in `test/ui/snapshots/` |
| `test/ui/snapshots/*.png` | Committed reference popup images (single / multi / truncated event lists, the empty state, and the unsupported-host fallback-with-request and "Disagree?" states), browsable on GitHub |
