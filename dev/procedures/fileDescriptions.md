...
| `dev/routines/create-extractor/phase1-prepare.sh` | Records the event page via an inline curl→ScraperAPI (`scraperapi_fetch`) during the auto-extractor Phase-1 pipeline |
...
| `dev/routines/create-extractor/` | The auto-implement-extractor pipeline in one folder: the self-contained agent prompt, the deterministic Node steps (triage, probe, naming, derive-names, scaffold-source/case, add-supported-domain, case-quality), and the workflows' phase scripts (`phase1-prepare.sh`, `handoff-to-agent.sh`, `phase2-finalize.sh`). A three-stage label relay: prepare workflow → Claude Code web routine → finalize workflow. See `dev/routines/create-extractor/auto-extractor.md` |
| `dev/routines/create-extractor/triage-extractor-request.js` | Auto-extractor pre-flight: detects a request whose host is already allow/denylisted so the workflow can close it before the agent runs |
| `dev/routines/` | Home for the repo's unattended routines: the auto-implement-extractor pipeline (`create-extractor/`) and the daily incremental-maintenance routines that improve the shipped extractor over time (currently `auto-fallback-coverage/`, which widens the generic fallback extractor's field coverage — the gate itself lives under `dev/requirements/extractor/fallback/`) |
