...
auto-extractor pipeline's probe (`dev/tools/new-extractors-creation/probe-url.js`), the
...
harness `extension-test/harness.js` stays under `test/` since the unit tests share it too.
...
- `npm run refresh:ui` — regenerate the `ui/cases/*.png` snapshots + the inline
  ...
  **Never** silently re-baseline a moved snapshot — surface the visual diff for
  approval (see [`dev/procedures/claude/workflow.md`](../procedures/claude/workflow.md)).
...
[`dev/procedures/claude/adding-a-source.md`](../procedures/claude/adding-a-source.md). It adds a
...
tracked — see the banner in `requirements.md` and `dev/procedures/claude/testing.md`.
