...
| `dev/incremental-maintenance/auto-fallback-coverage/routine.md` | The lean agentic spec: run preconditions, attempt one generic win, run postconditions, open a PR. All deterministic mechanics (and their rationale) live in the two sibling scripts, not this prose |
| `dev/incremental-maintenance/auto-fallback-coverage/preconditions.sh` | Pre-work script: exits non-zero (skip the routine) unless a meaningful non-docs/non-generated change landed on the base branch in the last 24h; otherwise establishes the baseline (`npm install` + `test:live`) and snapshots the pre-change coverage numbers for postconditions to compare against |
| `dev/incremental-maintenance/auto-fallback-coverage/postconditions.sh` | Post-work validation of a candidate win: diff-scope, full `npm test`, a real improvement over the baseline snapshot, and (per value arg) the jsdom-artifact check. Any non-zero exit means the run failed and must not retry or open a PR |
