...
// requirement. Each case (executable-requirements/ui/cases/<name>.case.js) supplies fake data;
...
// reference PNG (executable-requirements/ui/cases/<name>.png). So the
...
// has no pixels and is verified by executable-requirements/ui/events-view-actions.test.js instead.
const CASES = loadCases().filter(rendersImage);

test("there is at least one UI case", () => {
  assert.ok(CASES.length > 0, "no image-producing executable-requirements/ui/cases/*.case.js found");
});
