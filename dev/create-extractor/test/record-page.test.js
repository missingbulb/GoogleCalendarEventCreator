...
// #587 (eventer): the default fetch asks ScraperAPI to render JS, but a server-rendering
// host gets an empty SPA shell from render=true — so it's fetched WITHOUT render.
test("the default fetch requests JS rendering (render=true)", () => {
  const r = runRecordPage("standard:html");
  assert.equal(r.status, 0, r.stderr);
  assert.match(r.urls[0], /(\?|&)render=true&/);
});

test("eventer.co.il (server-rendered) is fetched WITHOUT render", () => {
  const r = runRecordPage("standard:html", "https://www.eventer.co.il/8lfff");
  assert.equal(r.status, 0, r.stderr);
  assert.doesNotMatch(r.urls[0], /render=true/); // render=true would return the empty shell
  assert.match(r.urls[0], /country_code=il&/);    // geo-targeting still applies (.il host)
});
