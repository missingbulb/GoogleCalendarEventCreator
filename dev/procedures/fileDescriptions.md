...
| `dev/routines/` | The legacy home of the repo's unattended routines, mid-conversion to gcec pack tasks: `create-extractor/` (implements a site extractor from an `extractor-request` issue) is still here; `fallback-extractor-improvements/` has moved to `.claudinite/local/packs/gcec/tasks/fallback-extractor-improvements/` |
