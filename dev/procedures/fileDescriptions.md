...
| `dev/incremental-maintenance/auto-fallback-coverage-precondition.sh` | Precondition guard the fallback-coverage routine runs first: exits 0 to proceed when a meaningful (non-docs/non-generated) code change landed on the base branch in the last 24h, else exits 1 and prints the skip reason. Keeps the deterministic gate — and its full rationale, in header comments — out of the routine prose |
