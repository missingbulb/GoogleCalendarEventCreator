...
The shared canon arrives here as the read-only submodule `docs/claude/shared/`,
...
- **Removes** the now-duplicated local item (typically from the local working-set
  docs `docs/engineeringPractices.md` / `docs/agenticBestPractices.md`, but any
...
- Compare local docs against the **currently pinned** submodule canon, not a live
  ...
  keeps it current.
- Never edit `docs/claude/shared/` (read-only) and never merge anything itself.
