...
for (const testCase of extractorCases) {
  ...
  test(`${id}: ${testCase.host} is recognized as supported and yields a complete event (${testCase.page})`, () => {
    ...
    assert.ok(
      ...
      `missing/empty cached page for ${testCase.host}: ${htmlPath} (recorded by the auto-extractor pipeline / by hand via ScraperAPI — see dev/routines/create-extractor/phase1-prepare.sh)`
    );
