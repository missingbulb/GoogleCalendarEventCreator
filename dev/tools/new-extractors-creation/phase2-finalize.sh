...
CASE_FILE="dev/requirements/extractors/custom/$CASE_NAME.json"
...
The workflow scaffolded the branch — the \`matches()\` gate, the \`supportedDomains\` entry, the regenerated load lists, and the real cached event page (\`dev/requirements/data/$CASE_NAME.html\`). The agent wrote \`extract()\` and the integration case asserting the real extraction.
