...
//   - dev/requirements/data/<name>.url   — plain text, the source URL (single source of truth;
//                         live.test.js reads it too, to fetch and to set the
//                         DOM's origin).
//   - dev/requirements/data/<name>.html  — the recorded HTML the live tests assert against.
//
// A cached HTML file is fetched when:
//   - --force was given, OR
//   - dev/requirements/data/<name>.html is missing or empty (zero bytes).
// A committed empty (zero-byte) dev/requirements/data/<name>.html is the "fetch me" signal: pair
// it with a dev/requirements/data/<name>.url and the next refresh fills it in. That's also the
// pre-case flow — record the HTML first, then add dev/requirements/extractors/custom/<name>.json
...
//
// Usage:
//   node dev/requirements/infra/data/refresh-cache.js            # fetch missing/empty cached HTML files only
//   node dev/requirements/infra/data/refresh-cache.js --force    # re-fetch everything
"use strict";
