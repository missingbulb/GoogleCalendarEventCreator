...
// dev/requirements/extractors/custom/*.json (live.test.js). A `tbd`
...
for (const testCase of extractorCases) {
  ...
  test(`${id}: ${testCase.host} is recognized as supported and yields a complete event (${testCase.page})`, () => {
    ...
    assert.ok(
      ...
      `missing/empty cached page for ${testCase.host}: ${htmlPath} (record it with node dev/requirements/infra/data/refresh-cache.js)`
    );
