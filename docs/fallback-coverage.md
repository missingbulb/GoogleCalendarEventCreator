...
Fields where the fallback produced a value that **differs** from the dedicated source (a wrong/weaker value, not merely a missing one) — the cases worth a look:

- **`bandsintown-berry-sakharof`**
  - `title` — custom: `Berry Sakharof` · fallback: `Berry Sakharof @ יער השלום Peace Forest`
- **`edinburghfringe-daniel-sloss`**
  - `start` — custom: `2026-08-14T20:00:00` · fallback: `2026-08-14T19:00:00`
- **`edinburghfringe-kristen-schaal`**
  - `start` — custom: `2026-08-12T19:00:00` · fallback: `2026-08-12T18:00:00`
- **`edinburghfringe-mr-chonkers`**
  - `start` — custom: `2026-08-10T23:55:00` · fallback: `2026-08-10T22:55:00`
- **`edinburghfringe-richard-herring-rhlstp`**
  - `start` — custom: `2026-08-05T14:00:00` · fallback: `2026-08-05T13:00:00`
- **`edinburghfringe-sophie-duker`**
  - `start` — custom: `2026-08-05T20:30:00` · fallback: `2026-08-05T19:30:00`
- **`eventbrite-1989775742810`**
  - `description` — custom: `תשלום מתבצע בלינק הבא-  https://smartbee.co.il/…` · fallback: `תשלום מתבצע בלינק הבא- https://smartbee.co.il/p…`
- **`eventbrite-games-for-change`**
  - `description` — custom: `Play, learn, and connect at NYC’s longest-runni…` · fallback: `Play, learn, and connect at NYC’s longest-runni…`
- **`eventim-co-il-the90sshow`**
  - `description` — custom: `מופע שנות ה-90 מציג את "פסטיבל ניינטיז" ⏎ מופע …` · fallback: `מציג את "פסטיבל ניינטיז"`
- **`luma-event`**
  - `location` — custom: `Florentin, Tel Aviv-Yafo, Israel` · fallback: `Tel Aviv-Yafo, Israel, Tel Aviv District`
- **`meetup-fusion-la-israel`**
  - `description` — custom: `​[Fusion VC](https://www.fusion-vc.com/?utm_sou…` · fallback: `​[Fusion VC](https://www.fusion-vc.com/?utm_sou…`
- **`meetup-nyc-tech-mixer`**
  - `description` — custom: `**JOIN US FOR THE BEST NETWORKING EVENT IN NYC!…` · fallback: `**JOIN US FOR THE BEST NETWORKING EVENT IN NYC!…`
- **`meetup-startup-designers`**
  - `description` — custom: `**התגעגענו והגיע הזמן שניפגש! 💜** ⏎ העולם שלנו…` · fallback: `**התגעגענו והגיע הזמן שניפגש! 💜** ⏎ העולם שלנו…`
- **`secrettelaviv-world-cup-eve`**
  - `start` — custom: `2026-06-16T18:30:00` · fallback: `2026-06-16`
  - `description` — custom: `Get ready for World Cup Eve with Kaduregel Shef…` · fallback: `World Cup Eve with talks, food, and drinks ⚽️`
- **`thinkdrink-quantum-lecture`**
  - `start` — custom: `2026-06-15T18:30:00` · fallback: `2026-06-15`
  - `description` — custom: `מה קורה למציאות כשאף אחד לא מסתכל עליה? ⏎ מסע א…` · fallback: `מה קורה למציאות כשאף אחד לא מסתכל עליה?מסע אל מ…`
