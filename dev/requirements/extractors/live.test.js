...
// data/, recorded from each site by dev/requirements/infra/data/refresh-cache.js (see also
...
//
// Each JSON file in dev/requirements/extractors/custom/ describes one scenario. The
...
// dev/requirements/data/<name>.url (the single source of truth — refresh-cache.js fetches it,
...
//
// To cover a new website or platform: add a dev/requirements/data/<name>.url with the event
// page URL and a case file (dev/requirements/extractors/custom/<name>.json) with its
// `expected`, then record the cached HTML with `node dev/requirements/infra/data/refresh-cache.js`
...
for (const file of caseFiles) {
  ...
  test(`${testCase.description || file} — ${url}`, () => {
    ...
    assert.ok(
      ...
      `Missing cached HTML for "${name}". Record it with: node dev/requirements/infra/data/refresh-cache.js`
    );
