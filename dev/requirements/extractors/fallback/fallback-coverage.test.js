...
//
// The comparison itself lives in dev/requirements/extractors/fallback/fallback-coverage.js (run GCal.extract()
...
//     dev/requirements/extractors/fallback/fallback-coverage.baseline.GENERATED.json — compared over the cases the run and the
...
//     is still caught.
//   - A REFRESH test (skipped in CI) rewrites dev/requirements/extractors/fallback/fallback-coverage.GENERATED.md and the
...
const BASELINE_COMMENT =
  ...
  "fallback-coverage.test.js and explained in dev/requirements/extractors/fallback/fallback-coverage.GENERATED.md. criticalFieldsPct/" +
...
test("fallback coverage has not regressed below the high-watermark", (t) => {
  ...
  if (gate.removed.length) {
    const msg =
      ...
      `re-anchored in your working tree — commit the updated dev/requirements/extractors/fallback/fallback-coverage.baseline.GENERATED.json.`;
  ...
  for (const key of GATED) {
    assert.ok(
      ...
      `Fallback ${key} dropped to ${gate.current[key]}% over the ${gate.shared.length} cases shared ` +
        ...
        `the dedicated sources — see dev/requirements/extractors/fallback/fallback-coverage.GENERATED.md and the value differences printed above. ` +
        `If this is an intentional, reviewed change, lower ${key} in ` +
        `dev/requirements/extractors/fallback/fallback-coverage.baseline.GENERATED.json.`
    );
  }
});
