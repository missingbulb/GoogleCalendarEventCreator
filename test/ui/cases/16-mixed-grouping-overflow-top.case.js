// Many instances of EVERY grouping style stacked in one overflowing list, at rest
// at the top of scroll — the visual stress test for a heterogeneous listing
// (#339). The list packs, in order: plain single cards, a same-day card (several
// times on one day -> one card, a button per time), a scattered-date month card
// with a shared start time (common-time header), an all-day month card, and more
// singles — so cards of very different heights sit side by side. It overflows the
// height cap, so only the bottom fade shows (more below).
"use strict";
