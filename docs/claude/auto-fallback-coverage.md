...
`data/*.html` fixtures (the environment may block live fetches) — and **most runs
...
Just the **generic** extractor: `pipeline/extract-unsupported.js` and the shared
`pipeline/helpers/*.js`. *Generic* means a change that keys off a **widely-used**
...
`pipeline/sources/<site>.js`'s logic into the fallback, or special-casing a host —
if the only beneficiary is a single case and the rule wouldn't fire elsewhere, it
isn't generic.

## How it measures (the gate)

The fallback-coverage gate lives in `test/extractors/fallback/` and runs in
...
- **(b) deliberate decisions.** Some misses are intentional and pinned by a unit
  ...
  all-day, in `test/unit/extraction.test.js`). Never "fix" what a test
...
test in `test/unit/extraction.test.js`; full suite green (`npm test`) and
