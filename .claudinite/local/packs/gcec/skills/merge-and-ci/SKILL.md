...
- **`enable_pr_auto_merge` has a narrow window — read its refusal as state, not
  as failure.** GitHub only arms auto-merge while a required check exists **and
  is still pending**. Before the branch's first `test` run registers it refuses
  *"unstable status (required checks are failing)"*; once that run completes
  green it refuses *"already in clean status … you can merge directly."*
  Neither is an error to escalate — the first means the run hasn't appeared yet,
  so poll on the back-off above and retry; the second means the merge is
  available **now**, so squash-merge it, which is the same end state the arming
  was for. **A green, conflict-free PR that merely failed to arm is never
  ...
  hand-cleared escalation**, on a change nothing was wrong with.
